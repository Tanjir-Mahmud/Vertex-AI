...
- **Text & URL Fact-Checking:** Leverages **You.com Search API** for real-time web grounding, feeding context into **Google Gemini 2.5 Flash** to evaluate accuracy and detect AI-generated phrasing.
- **Image Deepfake Detection:** Directly analyzes images using **Gemini 2.5 Flash (Vision)** with specialized forensic prompts looking for anatomical anomalies, boundary artifacts, and unnatural lighting.
...
graph TD
    ...
    YouSearch -->|Web Context Snippets| Gemini[Google Gemini 2.5 Flash]
    
    Condition -->|image| GeminiVision[Google Gemini 2.5 Flash Vision]
    GeminiVision -->|Forensic Prompt + Base64| GeminiVision
    
    Gemini --> Normalizer[Data Normalization]
    GeminiVision --> Normalizer
...
3.  **Branching Logic:**
    -   *Text/URL:* Fetches recent context via `https://api.ydc-index.io/search`. Passes the original physical input + web context to `gemini-2.5-flash` instructed as a forensic journalist.
    -   *Image:* Bypasses web search. Sends the base64 image via Data URI to `gemini-2.5-flash` instantiated as a visual forensic analyst looking for deepfake anomalies.
...
- **AI/APIs:** Google Gemini (`gemini-2.5-flash` text & vision model), You.com Search API.
- **Language:** TypeScript.
...
1.  **Clone the Repository:**
    ```bash
    git clone https://github.com/Tanjir-Mahmud/Vertex-AI.git
    cd Vertex-AI
    ```
...
-   **Secret Management:** API keys are never exposed to the client. All API calls to You.com and Gemini happen server-side inside the `/api/analyze` Route Handler.
